...
10. Line 16 (revert): during each iteration, the element at the `i-th - 1` position of `result` is replaced with the element at the `input size - i` position of input list.
