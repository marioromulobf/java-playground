...
1. Line 6: a variable `countries` of type Array of Country is initialized with six Country objects, each containing a country name and its corresponding language.
...
8. Line 25 (main): the DPK04Impl9 class is instantiated.
9. Line 27 (main): the `patternMatcher` method is called with the input `Usa`.
10. Line 16 (patternMatcher): create a sequential ordered stream from `countries` array of Country objects.  
