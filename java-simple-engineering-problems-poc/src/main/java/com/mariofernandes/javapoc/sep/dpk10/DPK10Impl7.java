...
public class DPK10Impl7 {

    public interface Reducer<A, I> {
        A apply(A accumulator, I item);
    }

    public interface Sequence<I, A> {
        boolean hasNext();
        I next();

        default A reduce(Reducer<A, I> reducer, A initial) {
    ...
    public static class ArraySequence<I, A> implements Sequence<I, A> {
    ...
    public static class CollectionSequence<I, A> implements Sequence<I, A> {
    ...
    public static void main(String[] args) {
        System.out.println("--> DPK 10 - Implementation 07 <--");
        DPK10Impl7 dpk10Impl7 = new DPK10Impl7();

        var input1 = new ArraySequence<Integer, Integer>(new Integer[]{1, 2, 3, 4, 5});
        var result1 = input1.reduce((acc, x) -> acc + x, 0);
        System.out.println("reduce([1, 2, 3, 4, 5], (acc, x) -> acc + x, 0) -> " + result1);

        var input2 = new ArraySequence<String, String>(new String[]{"a", "b", "c", "d", "e"});
