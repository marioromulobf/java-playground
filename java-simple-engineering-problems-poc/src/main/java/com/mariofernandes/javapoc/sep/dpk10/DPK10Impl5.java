...
public class DPK10Impl5 {
    ...
    public<A, I> A reduce(Sequence<I> items, Reducer<A, I> reducer, A initial) {
        validateItems(items);
        ...
        while (items.hasNext()) {
            accumulator = reducer.apply(accumulator, items.next());
        }

        return accumulator;
    }

    private static<I> void validateItems(Sequence<I> items) {
        if (items == null) {
    ...
    public static void main(String[] args) {
        System.out.println("--> DPK 10 - Implementation 05 <--");
        DPK10Impl5 dpk10Impl5 = new DPK10Impl5();

        var input1 = new ArraySequence<>(new Integer[]{1, 2, 3, 4, 5});
        var result1 = dpk10Impl5.reduce(input1, (acc, x) -> acc + x, 0);
        System.out.println("reduce([1, 2, 3, 4, 5], (acc, x) -> acc + x, 0) -> " + result1);

        var input2 = new ArraySequence<>(new String[]{"a", "b", "c", "d", "e"});
        var result2 = dpk10Impl5.reduce(input2, (acc, x) -> acc + x, "");
